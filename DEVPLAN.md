...
**Goal:** Add ForgeCode — the post-leak, Apache-2.0, Rust-native coding agent from Tailcall Inc. that currently leads Terminal-Bench 2.0 (#2 with GPT-5.4 at 81.8%, #4 with Claude Opus 4.6 at 79.8%) — as the 6th launcher in crazycode. ForgeCode is a candidate runtime evaluated for replacing Claude Code on a production project, and crazycode is the surface used to exercise it side-by-side with the other assistants. Final menu order (alphabetical, per M18 convention): `aider · claude · codex · forge · gemini · opencode`.
